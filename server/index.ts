...
app.get('/api/items/:id/progress', async (request, reply) => { if (requireAuth(request, reply)) return; const id = Number((request.params as { id: string }).id); const item = db.prepare('SELECT path FROM media_items WHERE id = ?').get(id) as { path: string } | undefined; if (!item) return reply.code(404).send({ error: 'Archivo no encontrado' }); const state = db.prepare('SELECT position, watched FROM playback_by_path WHERE path = ?').get(item.path) as { position: number; watched: number } | undefined; return { position: state?.position ?? 0, watched: Boolean(state?.watched) }; });
